...
class DataProcessor:
    ...
    def unzip_and_extract(self) -> bool:
        """
        İndirilen ZIP dosyasını açar. (Sıradaki adım)
        """
        print(f"[{self.username}]: ZIP dosyasını açma işlemi bekleniyor...")
    ...
    def run_analysis(self) -> dict:
        ...
        """
        base_path = os.path.join(self.extracted_path, "connections", "followers_and_following")

        if not os.path.exists(base_path):
            self.analysis_results = {"error": "followers_and_following klasörü bulunamadı."}
            return self.analysis_results

        # Dosya isimleri ve analiz etiketleri
        analysis_targets = {
            "followers_1.json": "Takipçi sayısı",
            "following.json": "Takip edilen sayısı",
            "blocked_profiles.json": "Engellenen hesap sayısı",
            "hide_story_from.json": "Hikaye gizlenen hesap sayısı",
            "recently_unfollowed_profiles.json": "Yakın zamanda takipten çıkılan hesap sayısı",
            "following_requests_you've_received.json": "Alınan takip isteği sayısı",
            "recent_follow_requests.json": "Yakın zamanda gönderilen takip isteği sayısı"
        }

        summary = {}

        for filename, label in analysis_targets.items():
            file_path = os.path.join(base_path, filename)
            if os.path.exists(file_path):
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        data = json.load(f)
                        # JSON verisinin türüne göre kayıt sayısı hesapla
                        if isinstance(data, list):
                            count = len(data)
                        elif isinstance(data, dict):
                            # Bazı JSON’lar {"relationships_following": [..]} gibi olabilir
