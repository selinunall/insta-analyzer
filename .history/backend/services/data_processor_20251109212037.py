...
class DataProcessor:
    ...
    def _load_json_data(self, relative_path: str) -> list:
        ...
        try:
            with open(full_path, 'r', encoding='utf-8') as f:
                # Instagram JSON formatı bazen tek bir anahtar altında listeyi tutar
                data = json.load(f)
                # Genellikle ilk anahtar, listenin adıdır
                if isinstance(data, dict) and data:
                    # 'followers' veya 'relationships_followers' gibi anahtarı alıyoruz
    ...
    def run_analysis(self) -> dict:
        ...
        # 1. Verileri Yükle
        # Format: [{'value': 'username', 'timestamp': 1234567890}] listesi bekleniyor.
        ...
        # 'followers_and_following' altındaki dosyalar genellikle düz liste yerine 
        # 'relationships_followers' veya 'followers' gibi bir anahtar altında veri tutar.
        # Bu yüzden _load_json_data fonksiyonu listeyi doğrudan döndürmeye çalışır.

        # 2. Toplam Sayıları Hesapla
        total_followers = len(followers_data)
        total_following = len(following_data)

        # 3. Karşılıklı Takip Etmeyenleri Bul (Basit Versiyon)
        # Sizin takip ettikleriniz (following) - Sizi takip edenler (followers)
        
        # 'following.json' yapısı: [{'value': 'username', 'timestamp': ...}] veya daha karmaşık.
        # En basit şekilde kullanıcı adlarını (value) alalım.
        followers_list = {item.get('value') for item in followers_data if item.get('value')}
        following_list = {item.get('value') for item in following_data if item.get('value')}

        # Sizin takip edip, onların sizi takip etmediği kişiler (Unfollowers/Not Following Back)
        not_following_back = following_list - followers_list
        
        # İlk 10 karşılıklı takip etmeyeni gösterelim (ya da bir sayıyı)
        unfollowers_count = len(not_following_back)
        top_unfollower = next(iter(not_following_back), "Yok")
        
        # 4. Sonuçları Hazırla (Frontend'deki arayüze uyması için alan isimlerini kullanıyoruz)
        self.analysis_results = {
            "TakipçiSayısı": total_followers, # Toplam Takipçi Sayısı
            "TakipEdilenSayısı": total_following, # Toplam Takip Edilen Sayısı
            "GeriTakipEtmeyenSayısı": f"GT Yapmayan Sayısı: {unfollowers_count}", # Unfollower Sayısı/Mesajı
            "İlkGeriTakipEtmeyen": f"İlk GT Yapmayan: {top_unfollower}" # İlk Unfollower
        }
        
        print(f"[{self.username}]: Analiz Tamamlandı. Takipçi: {total_followers}, Takip Edilen: {total_following}")
