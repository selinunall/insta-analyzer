...
class DataProcessor:
    ...
    def _load_json_data(self, relative_path: str) -> list:
        ...
        if not os.path.exists(full_path):
            ...
            # 2. Esnek Klasör Araması Dene
            print(f"[{self.username}]: '{relative_path}' doğrudan yolda bulunamadı. Alternatif klasör aranıyor...")
            
            # self.extraction_path altındaki tüm öğeleri listele
            content_list = os.listdir(self.extraction_path)
            
            # Tek bir alt klasör varsa ve bu alt klasör bir dizinse, oraya in
            sub_dirs = [d for d in content_list if os.path.isdir(os.path.join(self.extraction_path, d))]

            if len(sub_dirs) == 1:
                # Olası tarih/kullanıcı adı içeren ana klasör
                single_sub_dir = sub_dirs[0]
                
                # Yeni yolu oluştur: extraction_path/single_sub_dir/relative_path
                ...
                if os.path.exists(potential_path):
                    ...
                    # Alternatif yolda da bulunamadı
                    print(f"[{self.username}]: Uyarı: Dosya bulunamadı: {relative_path}")
                    return []
            else:
                # Birden fazla alt klasör var veya hiç alt klasör yok, dosya bulunamadı
                print(f"[{self.username}]: Uyarı: Dosya bulunamadı: {relative_path}")
                return []
        
        # 3. JSON Okuma ve Veri Çıkarma (Önceki mantık)
        try:
            ...
            print(f"[{self.username}]: Hata: JSON okunamadı ({full_path}): {e}")
            return []
    ...
    def run_analysis(self) -> dict:
        ...
        followers_data = self._load_json_data('connections/followers_1.json')
        following_data = self._load_json_data('connections/following.json')
