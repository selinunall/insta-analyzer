...
type AppState = 'HOME' | 'REQUEST_WEBVIEW' | 'PASTE_LINK' | 'DOWNLOAD_WEBVIEW' | 'PROCESSING' | 'ANALYSIS_RESULTS';
...
export default function App() {
  ...
  useEffect(() => {
    if (appState === 'PROCESSING' && finalDataUrl) {
      console.log("Veri indirme ve analiz simülasyonu başlatılıyor...");
      
      // Gerçek bir uygulamada, bu kısımda finalDataUrl bir backend servisine gönderilirdi.
      // Backend: 1. URL'den ZIP dosyasını indirir, 2. Arşivden verileri (JSON) çıkarır, 3. Analiz yapar.
      
      // Simülasyon: 5 saniye bekletme
      const simulationTimer = setTimeout(() => {
        const mockResults: InstagramAnalysisResult = {
          totalPosts: 124,
          mostLikedPost: "Brezilya tatilinden manzara fotoğrafı (1500 beğeni)",
          averageLikes: 450.7,
          topFollowerCountry: "Türkiye",
        };

        setAnalysisResults(mockResults);
        setAppState('ANALYSIS_RESULTS');
        console.log("Analiz simülasyonu tamamlandı. Sonuçlar yüklendi.");

      }, 5000); // 5 saniye bekleme
      
      // Component unmount olduğunda veya state değiştiğinde temizleme
      ...
      case 'ANALYSIS_RESULTS':
        return (
            <ScrollView style={styles.container} contentContainerStyle={styles.content}>
                ...
                <Text style={styles.description}>
                    Kullanıcınıza ait veriler başarıyla analiz edildi.
                </Text>

                {analysisResults ? (
                    <View style={styles.resultsCard}>
                        <Text style={styles.resultItemTitle}>Toplam Gönderi Sayısı:</Text>
                        <Text style={styles.resultItemValue}>{analysisResults.totalPosts}</Text>
                        
                        <View style={styles.separator} />

                        <Text style={styles.resultItemTitle}>Ortalama Beğeni Sayısı:</Text>
                        <Text style={styles.resultItemValue}>{analysisResults.averageLikes.toFixed(1)}</Text>
                        
                        <View style={styles.separator} />

                        <Text style={styles.resultItemTitle}>En Çok Beğeni Alan Gönderi:</Text>
                        <Text style={styles.resultItemValue}>{analysisResults.mostLikedPost}</Text>

                        <View style={styles.separator} />
                        
                        <Text style={styles.resultItemTitle}>Takipçilerin En Yoğun Olduğu Ülke:</Text>
                        <Text style={styles.resultItemValue}>{analysisResults.topFollowerCountry}</Text>
                        
                    </View>
                ) : (
                    <Text style={styles.description}>Sonuçlar yüklenemedi. Lütfen tekrar deneyin.</Text>
                )}
