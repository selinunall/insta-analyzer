...
interface InstagramAnalysisResult {
    TakipçiSayısı: number; // YENİ ANLAM: Toplam Takipçi Sayısı
    GeriTakipEtmeyenSayısı: string; // YENİ ANLAM: GT Yapmayan Sayısı/Mesajı
    TakipEdilenSayısı: number; // YENİ ANLAM: Toplam Takip Edilen Sayısı
    İlkGeriTakipEtmeyen: string; // YENİ ANLAM: İlk GT Yapmayan
}
      ...
      const sendDataToBackend = async () => {
        ...
        try {
          ...
          if (response.ok && data.status === 'success') {
            // Başarılı yanıt geldi
            console.log("Backend'den analiz sonuçları başarıyla alındı.");
            setAnalysisResults(data.results as InstagramAnalysisResult);
      ...
      case 'HOME':
        return (
          <View style={styles.content}>
            ...
            <Text style={styles.description}>
              E-posta adresinize bilgilendirme maili geldiğinde, hazır olan dosyanızı indirmek için butona tıklayın.
            </Text>
    ...
    case 'REQUEST_WEBVIEW':
      ...
      case 'DOWNLOAD_WEBVIEW':
        return (
          ...
          <View style={styles.webviewContainer}>
            <WebView
              // GÜNCELLENDİ: Artık doğrudan Accounts Center URL'sini açıyoruz.
              source={{ uri: ACCOUNTS_CENTER_DATA_URL }} 
              style={[styles.webview, { zIndex: 0 }]}
              startInLoadingState={true}
            ...
            <ScrollView style={styles.container} contentContainerStyle={styles.content}>
                ...
                {analysisResults ? (
                    <View style={styles.resultsCard}>
                        <Text style={styles.resultItemTitle}>Toplam Takipçi Sayısı:</Text>
                        <Text style={styles.resultItemValue}>{analysisResults.TakipçiSayısı}</Text>
                        
                        <View style={styles.separator} />

                        <Text style={styles.resultItemTitle}>Toplam Takip Edilen Sayısı:</Text>
                        <Text style={styles.resultItemValue}>{analysisResults.TakipEdilenSayısı.toFixed(1)}</Text>
                        
                        <View style={styles.separator} />

                        <Text style={styles.resultItemTitle}>Karşılıklı Takip Etmeyenler:</Text>
                        <Text style={styles.resultItemValue}>{analysisResults.GeriTakipEtmeyenSayısı}</Text>

                        <View style={styles.separator} />
                        
                        <Text style={styles.resultItemTitle}>İlk Geri Takip Etmeyen Kullanıcı:</Text>
                        <Text style={styles.resultItemValue}>{analysisResults.İlkGeriTakipEtmeyen}</Text>
