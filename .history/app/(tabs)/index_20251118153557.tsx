...
export default function App() {
  ...
  PushNotification.configure({
    // (Gerekli) Bir bildirim alındığında (ön plan/arka plan) çalışır
    onNotification: function (notification) {
      console.log("NOTIFICATION:", notification);
      // İşleme mantığınız buraya gelir
    },

    // (Opsiyonel) Bildirim izinlerini ister (sadece iOS)
    requestPermissions: Platform.OS === 'ios',
  });

  // Android için bir kanal oluşturun
  PushNotification.createChannel(
    {
      channelId: "email-check-reminder", // Kanal ID'si (benzersiz olmalı)
      channelName: "E-posta Kontrol Hatırlatıcısı", // Kullanıcıya gösterilecek isim
      channelDescription: "Kullanıcıya e-posta kontrolünü hatırlatır.", 
      importance: 4, // Yüksek önem seviyesi
      vibrate: true,
    },
    (created) => console.log(`Kanal 'email-check-reminder' ${created ? 'başarıyla oluşturuldu' : 'zaten mevcuttu'}`)
  );
};
   const scheduleEmailCheckReminder = () => {
  // Şu anki zamana 1 saat (60 dakika) ekliyoruz
  const oneHourLater = new Date(Date.now() + (1 * 60 * 1000)); 

  PushNotification.localNotificationSchedule({
    channelId: "email-check-reminder", // Yukarıda tanımladığınız kanal ID'si
    title: "⏳ Verileriniz Hazır Olabilir!",
    message: "İnstagram'dan gelen veri indirme e-postasını kontrol etmeyi unutmayın.",
    date: oneHourLater, // Bildirimin tetikleneceği tarih ve saat
    allowWhileIdle: true, // Cihaz boşta (idle) olsa bile bildirim gönder
  });

  console.log("E-posta kontrol hatırlatıcısı 1 saat sonrasına ayarlandı.");
};

  const handleStep2EmailCheck = () => {
  // Önce durumu güncelleyin (varsa)
  // setAppState('DOWNLOAD_WEBVIEW'); // İlgili aksiyon
  
  // Ardından hatırlatıcıyı planlayın
  scheduleEmailCheckReminder();

  // Kullanıcıya bir geri bildirim mesajı gösterebilirsiniz
  Alert.alert("Hatırlatıcı Ayarlandı", "1 saat sonra e-posta kutunuzu kontrol etmeniz için size bir bildirim göndereceğiz.");
};

// YENİ FONKSİYON: Analiz kartına tıklandığında çağrılır
  const handleAnalysisCardPress = (index: number) => {
    setActiveAnalysis(index + 1); // 1'den başlayan indeksi kaydet
  ...
  const renderContent = () => {
    switch (appState) {
      ...
      const homeMetrics = analysisResults ? analysisResults.all_metrics : {
                ...
                // HOME sayfasındaki 10 analiz kartının verisi
                const analysisData = [
      ...
      <ScrollView style={styles.pageContent}> 
        <View style={styles.container}>
          ...
          <View style={styles.inputSection}>
                    {/* Başlık ile aynı dikey hizadan başlaması için, eğer styles.sectionTitle'ın margin'i yoksa, 
                       burada container'ın padding'ini kullanmalısınız. 
                       styles.container'ın başlangıcına dikkat edin. */}
          ...
          <View style={styles.stepCard}>
            ...
            <View style={styles.buttonWrapper}>
            <Button title="Hatırlat" onPress={handleStep2EmailCheck} color="#0927eb" />
            </View>
          </View>
        ...
        return (
          <View style={styles.webviewContainer}>
            ...
            <View style={styles.webviewHeader}>
              ...
              <Button title="Geri" onPress={handleGoBack} color="#0927eb" /> 
              <TouchableOpacity 
              style={styles.infoButtonWebView} 
  ...
  const renderAnalysisInfoModal = () => {
    ...
    // Tıklanan metriğin başlığını bulmak için, HOME'daki analizi tekrar tanımlıyoruz
    const analysisData = [
        { title: "Geri Takip Etmeyenler", key: 'not_following_back_count' },
        { title: "Karşılıklı Takip Sayısı", key: 'mutual_following_count' },
        { title: "Sizin Takip Etmedikleriniz", key: 'you_not_following_count' },
    ...
    // activeAnalysis 1'den başladığı için index'i bulurken 1 çıkarıyoruz
