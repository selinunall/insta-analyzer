...
const scheduleEmailCheckReminder = async () => {
  // --- 1. İzinleri Kontrol Etme (Expo için zorunlu) ---
  ...
  if (status !== 'granted') {
    Alert.alert(
      "İzin Gerekli", 
      "Bildirim göndermek için izin vermelisiniz. Lütfen uygulama ayarlarından bu izni etkinleştirin."
    );
    return;
  }
  
  // --- 2. Planlama (Test için 1 Dakika Sonra) ---
  // Gerçek kullanımda 1 saat sonrasına ayarlamak için: (60 * 60)
  const oneMinuteLater = new Date(Date.now() + (5 * 60 * 1000)); 
  const triggerSeconds = (oneMinuteLater.getTime() - Date.now()) / 1000; 

  await Notifications.scheduleNotificationAsync({
    content: {
      title: "⏳ Verileriniz Hazır Olabilir!",
      body: "İnstagram'dan gelen veri indirme e-postasını kontrol etmeyi unutmayın.",
      // data: { type: 'email_check' }, // İsteğe bağlı
    },
    trigger: {
      // Sürenin minimum 1 saniye olması için kontrol
      seconds: triggerSeconds > 0 ? triggerSeconds : 1, 
    } as any,
  });

  console.log("E-posta kontrol hatırlatıcısı 1 dakika sonrasına ayarlandı (Expo Test Modu).");
};
const handleStep2EmailCheck = () => {
  ...
  scheduleEmailCheckReminder(); // ✅ Bu çağrı yeni Expo fonksiyonunu tetikleyecek
