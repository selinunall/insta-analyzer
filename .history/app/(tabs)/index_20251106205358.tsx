...
// Bu değer artık bir sabit değil, stil olarak uygulanacak.
  ...
  const renderContent = () => {
    switch (appState) {
      ...
      case 'DOWNLOAD_WEBVIEW':
        return (
          // Ana 'container' bileşeninin esnekliği sayesinde, 
          // bu bileşenler dikey olarak sıralanır ve alan paylaşır.
          <View style={{ flex: 1 }}> 
            {/* Başlık: Artık WebView'dan tamamen ayrı ve üstte */}
            <View style={styles.webviewHeader}>
                <Text style={styles.webviewTitle}>Lütfen İndir Butonuna Tıklayın</Text>
                {/* Dokunma olayları artık WebView'ın baskısı altında değil. */}
                <Button title="Geri" onPress={handleGoBack} color="#FF4500" /> 
            </View>
            
            {/* WebView: Kalan tüm alanı doldurur. */}
            <WebView
              source={{ uri: downloadLink }}
              style={styles.webview} // marginTop kaldırıldı
...
const styles = StyleSheet.create({
  ...
  // webviewContainer stili artık kullanılmıyor
  webviewHeader: {
    flexDirection: 'row',
    ...
    borderBottomWidth: 1,
    borderColor: '#ddd',
    // Artık absolute position değil, normal akışta
    height: WEBVIEW_HEADER_HEIGHT, 
    zIndex: 1, // Yine de emin olmak için z-index bırakılabilir
    elevation: 5,
  },
  webviewTitle: {
    fontSize: 16,
